...
**Статус вех:** M1 ✅ · M2 ✅ · M3 ✅ · M4 ✅ · M5 почти закрыта (#39–#44 ✅, остаётся P3 #45) · M6 ✅ (инструменты сделаны; #49 — живой бэклог идей) · **M7 ✅** (раздел для владельцев VPN завершён: #70–#79) · **M8 📋 запланирована** (подготовка и публикация в App Store) · **M9 📋** (дизайн-редизайн: macOS + VPN + единый язык) · **M10 📋** (переводы и тон) · **M11 📋** (конкурентный отрыв и дизайн-лидерство).
...
## M7 · Инструменты для владельцев VPN — завершена ✅
...
сервера. Граница M7 соблюдена: диагностируем свой инбаунд и его выход, а не обходим DPI.

**#74 — просмотрщик mihomo `.mrs` (сделан, M7 закрыта).** Распаковывает скомпилированный rule-set
Clash.Meta/mihomo обратно в список доменов или подсетей. Формат разобран по исходникам mihomo: zstd-кадр →
контейнер `MRS\x01` + байт поведения (0 = домены, 1 = ipcidr) + счётчик + резерв → полезная нагрузка.
Домены хранятся **succinct-trie (LOUDS)** в обратном порядке (`com.example`) — `SuccinctDomainSet`
повторяет ридер и обход `keys()` mihomo, но с rank/select на popcount вместо предвычисленных индексов
(нам нужно один раз перечислить для показа). ipcidr — список слитых диапазонов [from,to] по 16 байт;
`IPRangeCIDR` разворачивает каждый диапазон в минимальные CIDR (на байтовой арифметике, без 128-битного
типа — держим планку iOS 17): IPv4-mapped рендерится как IPv4, остальное как IPv6. zstd Apple `Compression`
не умеет, поэтому вендорим декомпрессор-only `zstddeclib.c` (facebook/zstd 1.6.0, BSD) отдельной C-целью
`CZstd` и оборачиваем в `Zstd.decompress` (размер из кадра либо стриминг). Экран `MRSView` — открыть `.mrs`
(`fileImporter`) или по ссылке, шапка (тип, число правил, «исходных → после слияния»), поиск и экспорт
списка. Разбор покрыт юнит-тестами на реальных фикстурах из MetaCubeX/meta-rules-dat (домены и ipcidr,
IPv4+IPv6), а перечисление доменов сверено с независимым LOUDS-декодером (youtube.mrs → ровно 355 доменов;
telegram.mrs → 12 CIDR, включая IPv6). Граница соблюдена: только распаковка и просмотр списков, без
маршрутизации. **M7 закрыта** — весь раздел для операторов VPN (#70–#79) готов.
