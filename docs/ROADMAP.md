...
(версия ядра, `runXrayFromJson`, `stopXray`). `XrayProxyRunner.probeInProcess` собирает конфиг из
узла (`XrayTestConfig`: один локальный SOCKS-инбаунд + реальный outbound узла — при Xray-JSON берётся
verbatim, иначе восстанавливается из полей ссылки), поднимает ядро, ждёт открытия SOCKS-порта и через
него бьёт в gstatic `/generate_204` (`Socks5Probe`) — 204/2xx-3xx = инбаунд живой, иначе называем
причину. `XrayTestConfig`/`Socks5Probe`/`SubscriptionParser` покрыты юнит-тестами (сборка конфига из
reality-ссылки и из полного Xray-JSON — детерминированно). На iPhone 17 Pro (симулятор) подтверждено,
что **ядро линкуется, поднимается и отвечает** в процессе (`XrayCore.version()` → «Ядро Xray 26.7.11»
через `CGoInvoke`); экран `XrayCheckView` (`vless://`/`trojan://`/конфиг → карточка «инбаунд
работает / не прошло» с протоколом, сервером, задержкой и HTTP-кодом) рендерится. Позитивный
end-to-end-хендшейк проверяется на **своём живом сервере** — как on-device-проверка MTR и полные
прогоны теста скорости. Граница M7 соблюдена: диагностируем свой инбаунд, а не обходим DPI. **Осталось
