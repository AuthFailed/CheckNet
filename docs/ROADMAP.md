...
- **App + Shared** — ~11 000 строк; появился таргет **`CheckNetTests` (114 тестов)** на логику
...
**Live Activity подключена ко всем длящимся тестам.** Помимо пинга и мониторинга — **тест
скорости** (текущие Мбит/с + фаза загрузка/отдача), **bufferbloat** (фаза и RTT → итоговая оценка
A–F с цветом), **MTR** (задержка достигнутой цели, потери, раунд — непрерывно) и **трассировка**
(накопление хопов, достигнута ли цель). Каждому — свой чистый билдер `*ActivityContent` в
`Shared/` (юнит-тесты) плюс `kind` → иконка в виджете. Одноразовые проверки (DNS, TLS, whois,
host→IP…) намеренно без активности — там она была бы миганием на долю секунды. Найден и починен
race: `start()` этих моделей первым делом зовёт `stop()`, а асинхронный конец активности из
`stop()` гасил только что созданную — перешли на **отдельный контроллер на прогон**, захватываемый
таском. Проверено на симуляторе: MTR-активность держится в Dynamic Island (компакт «30 хопов» +
развёрнутый «Хопы/Потери/Раунд»).
