...
- **Локализация** — 680 ключей × 13 языков, покрытие 100 % для всего, что извлекает компилятор.
  Осталась вторая категория: строки моделей и движков, которые ищутся через `LocalizedStringKey(переменная)`
  и потому не извлекаются, — они по-прежнему только по-русски.
