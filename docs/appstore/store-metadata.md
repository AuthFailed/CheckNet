...
Иконка — это **дизайн-задача** (по `CLAUDE.md` иконки не рисуем ad-hoc в коде). Порядок:

1. Отдать промпт ниже в Claude Design, получить 1024×1024 (лучше — с исходником/вектором).
...
**Готовый промпт для Claude Design (вставить как есть):**
```
